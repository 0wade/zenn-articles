...
Webエンジニアとして必ず理解しておくべき HTTP の仕組みを、基礎から発展まで体系的にまとめました。概念の説明だけでなく、`curl` を使った実際の動作確認も交えて解説します。
...
HTTPは「記憶しない」プロトコルです。1リクエストごとに接続が独立しており、サーバーは前のリクエストを覚えていません。だからこそ **Cookie / Session / JWT** などが生まれました。
